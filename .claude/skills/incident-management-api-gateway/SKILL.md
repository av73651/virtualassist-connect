---
name: incident-management-api-gateway
description: "API Gateway incident patterns — 5xx/4xx spikes, latency, WAF blocks, integration timeouts, throttling."
---
...
|---------------|--------|-----------|
| `backend-lambda-error` | Delegate to Lambda addendum | AUTO (via Lambda) |
| `integration-timeout` | Delegate to Lambda addendum (duration) | AUTO (via Lambda) |
| `api-throttle` | Increase stage/usage plan limits | MANUAL |
| `waf-misconfiguration` | Update WAF rule set | MANUAL (security review) |
| `auth-issue` | Check Cognito pool, token TTL | MANUAL |
| `bad-deployment` | Redeploy previous stage | MANUAL |
