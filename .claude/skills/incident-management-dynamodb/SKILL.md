---
name: incident-management-dynamodb
description: "DynamoDB incident patterns — throttling, hot partitions, GSI issues, stream lag, capacity diagnostics."
---
...
|---------------|--------|-----------|
| `dynamo-capacity-exceeded` | Switch to on-demand / increase provisioned | MANUAL |
| `hot-partition` | Redesign partition key, add write sharding | MANUAL |
| `gsi-capacity-exceeded` | Increase GSI capacity | MANUAL |
| `stream-consumer-failure` | Fix consumer Lambda (delegate to Lambda addendum) | AUTO (via Lambda) |
| `capacity-ceiling` | Enable auto-scaling with target utilization 70% | MANUAL |

**Proposed addition to `remediation_catalog`:**
```json
"dynamo-capacity-exceeded": {
  "action": "dynamo-switch-to-ondemand",
  "description": "Switch table billing mode to PAY_PER_REQUEST"
}
```
