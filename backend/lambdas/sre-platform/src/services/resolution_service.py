...
class ResolutionService:
    ...
    def trigger_recovery(
        ...
        # Checkpoint-aware path: generate delta report instead of triggering workflow
        if catalog_entry.get("checkpoint_aware") and self._delta_report:
            return self._trigger_delta_report(
                service_name, jira_ticket_id, recovery_model,
            )
    ...
    def _trigger_delta_report(
        self, service_name: str, jira_ticket_id: str, recovery_model: str,
    ) -> tuple[str, str]:
        """Generate and post a checkpoint delta report instead of triggering a workflow."""
        result = self._delta_report.generate_and_post(
            service_name=service_name, jira_ticket_id=jira_ticket_id,
        )
        detail = (
            f"Delta report posted for {recovery_model}: "
            f"{result['checkpoints_found']} checkpoints, "
            f"{result['total_pending']} pending"
        )
        if result.get("zombies_found"):
            detail += f", {result['zombies_found']} zombies"
        return RecoveryStatus.DELTA_REPORTED, detail
