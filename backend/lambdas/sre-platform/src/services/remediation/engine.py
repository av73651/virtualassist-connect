...
class RemediationEngine:
    ...
    def attempt(
        ...
        if not skill:
            logger.warning(
                "No skill file for service_type=%s (loaded: %s)",
                service_type, list(self._skills.keys()),
            )
            return None
        ...
        if not entry:
            logger.warning(
                "No catalog entry for service_type=%s root_cause=%s "
                "(available: %s, ai_action=%s)",
                service_type, root_cause, list(catalog.keys()),
