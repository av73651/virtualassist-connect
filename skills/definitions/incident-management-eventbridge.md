...
|---------------|--------|------|---------|--------|
| `target-permission-error` | Fix Lambda resource-based policy | MANUAL | FailedInvocations > 0 + target Lambda not invoked; missing resource-based policy | FailedInvocations = 0, target Lambda receiving events |
| `target-failure` | Delegate to Lambda addendum | AUTO (via Lambda) | FailedInvocations > 0 + target Lambda throttled or erroring | FailedInvocations = 0, target Lambda healthy |
| `rule-throttle` | Consolidate rules, request limit increase | MANUAL | ThrottledRules > 0; too many rules matching same event pattern | ThrottledRules = 0, all matching rules executing |
| `dlq-depth-increasing` | Inspect, fix target, redrive DLQ | MANUAL | DLQ depth increasing over time; failed invocations or poison events | DLQ depth decreasing to 0, no new failed invocations |
| `pattern-mismatch` | Update event pattern on rule | MANUAL | MatchedEvents = 0 but source is publishing (verified via CloudTrail PutEvents) | MatchedEvents > 0, events flowing through rule |
| `stale-rule` | Update or delete rule target | MANUAL | Rule exists but target was deleted/changed; FailedInvocations on specific rule | FailedInvocations = 0 after target ARN updated |
