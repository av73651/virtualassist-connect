...
|---------------|--------|------|---------|--------|
| `data-loss-risk` | Investigate unassigned shards, restore from snapshot | MANUAL (critical) | ClusterStatus.red = 1, unassigned primary shards; node failure or disk full | ClusterStatus.green, all primary shards assigned, no data loss |
| `reduced-redundancy` | Scale data nodes, rebalance shards | MANUAL | ClusterStatus.yellow = 1, unassigned replica shards; insufficient nodes | ClusterStatus.green, all replicas assigned, zone-aware balanced |
| `disk-pressure` | Delete old indices, force merge | MANUAL | FreeStorageSpace < 20%, write rejections increasing; old indices consuming space | FreeStorageSpace > 35%, no write rejections, watermark cleared |
| `jvm-pressure` | Optimize queries, reduce shard count | MANUAL | JVMMemoryPressure > 80%, GC pauses; large aggregations or too many shards per node | JVMMemoryPressure < 75%, no long GC pauses |
| `write-saturation` | Reduce bulk size, scale data nodes | MANUAL | ThreadpoolWriteRejected > 0, indexing latency spike; bulk size too large or I/O saturated | WriteRejected = 0, indexing latency at baseline |
| `search-saturation` | Optimize queries, add data nodes | MANUAL | SearchLatency p99 spike, ThreadpoolSearchRejected > 0; expensive queries or insufficient nodes | SearchLatency p99 at baseline, SearchRejected = 0 |
| `backup-failure` | Fix S3 permissions, re-register snapshot repo | MANUAL | AutomatedSnapshotFailure = 1; S3 permission or repo registration issue | AutomatedSnapshotFailure = 0, latest snapshot successful |
